...
class LocalizationTest extends OpMode {
    @Override
    public void init() {
        follower.setStartingPose(new Pose(72,72)); //todo: check the encoder direcitons for pin point using forward test and strafe test...should fix issue?
    }
    ...
    public void loop() {
        ...
        if (!end) {
            if (Math.abs(follower.getPose().getX()) > (DISTANCE + 72)) {
                ...
                //double currentVelocity = Math.abs(follower.getVelocity().getXComponent()); //todo
                double currentVelocity = Math.abs(follower.poseTracker.getLocalizer().getVelocity().getX());
                velocities.add(currentVelocity);
                velocities.remove(0);
                telemetryM.debug("x: ", Math.abs(follower.poseTracker.getLocalizer().getVelocity().getX()));
                telemetryM.debug("y: ",  Math.abs(follower.poseTracker.getLocalizer().getVelocity().getY()));
                telemetry.update();
                follower.update();
            }
        } else {
